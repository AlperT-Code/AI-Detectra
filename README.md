<h1 align="center">🛰️ Detectra</h1>
<p align="center">An AI content detector — built with Node.js, Express &amp; the Groq API — that tells you whether a piece of text or an image was produced by artificial intelligence or a human, complete with a confidence score and concrete indicators. Ships as a multi-page web app with a modern, dark-themed interface.</p>
<p align="center">Node.js, Express ve Groq API ile geliştirilmiş bir yapay zeka içerik dedektörü — bir metnin veya görselin yapay zeka mı yoksa insan tarafından mı üretildiğini, bir güven skoru ve somut ipuçlarıyla birlikte söyler. Modern, koyu temalı çok sayfalı bir web uygulaması olarak çalışır.</p>
...
- **`public/`** — the front-end:
  - `index.html` — the five-page single-page app (home, tool, about, FAQ, terms), with every string tagged via `data-i18n` attributes
...
- **`baslat.bat`** — a Windows helper that starts the server and prints the local URL
...
4. Run it
   ...
   npm start
   ```
   Then open **http://localhost:3000** in your browser. On Windows you can also just double-click `baslat.bat`.
...
- **`public/`** — arayüz:
  - `index.html` — beş sayfalı tek sayfa uygulama (ana sayfa, araç, hakkımda, SSS, şartlar)
...
- **`baslat.bat`** — sunucuyu başlatan ve yerel adresi yazan Windows yardımcısı
...
4. Çalıştırın
   ...
   npm start
   ```
   Ardından tarayıcınızda **http://localhost:3000** adresini açın. Windows'ta `baslat.bat` dosyasına çift tıklamanız da yeterlidir.
